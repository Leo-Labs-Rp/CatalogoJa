...
create table public.tenants (
  ...
  constraint tenants_whatsapp_format_check check (whatsapp ~ '^[0-9]{10,15}$'),
...
-- Guarda os dados coletados antes do pagamento. Somente a service role acessa
-- esta tabela; o navegador passa por uma rota de API validada.
create table public.signup_intents (
  ...
  constraint signup_intents_whatsapp_check check (whatsapp ~ '^[0-9]{10,15}$'),
...
-- O id do evento é a chave primária para que reentregas do Asaas sejam
-- reconhecidas sem provisionar ou alterar uma assinatura duas vezes.
...
-- A leitura pública é feita por uma função com retorno limitado aos campos
-- necessários ao catálogo. owner_user_id e dados operacionais nunca são expostos.
