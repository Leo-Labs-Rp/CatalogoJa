...
export function SignupForm() {
  ...
  const effectiveSlugStatus = form.slug.length < 3 ? { available: false, checking: false, message: "Use pelo menos 3 caracteres." } : slugStatus;
      ...
      {step === 1 ? (
        <Card className="mx-auto w-full max-w-xl p-5 sm:p-7">
          <form className="grid gap-5" onSubmit={nextStep}>
            ...
            <div className="grid gap-4 sm:grid-cols-2"><Field><FieldLabel htmlFor="whatsapp">WhatsApp</FieldLabel><Input autoComplete="tel" id="whatsapp" inputMode="tel" maxLength={19} onChange={(event) => update("whatsapp", normalizeBrazilWhatsAppInput(event.target.value))} placeholder="+55 (11) 99999-9999" required type="tel" value={formatBrazilWhatsApp(form.whatsapp)} /><FieldDescription>Código do país + DDD + número. Salvamos apenas os dígitos.</FieldDescription></Field><Field><FieldLabel htmlFor="email">E-mail</FieldLabel><Input autoComplete="email" id="email" onChange={(event) => update("email", event.target.value)} placeholder="voce@empresa.com" required type="email" value={form.email} /><FieldDescription>Usado apenas para identificar o proprietário. Não enviaremos mensagens nesta fase.</FieldDescription></Field></div>
            <Field><FieldLabel htmlFor="slug">Endereço da loja</FieldLabel><div className="flex rounded-[var(--radius-control)] border bg-white focus-within:border-brand-600 focus-within:shadow-[var(--focus-ring)]"><span className="flex items-center border-r px-3 text-xs text-[var(--app-foreground-muted)]">/loja/</span><input className="h-11 min-w-0 flex-1 bg-transparent px-3 text-sm outline-none" id="slug" onChange={(event) => { setSlugEdited(true); update("slug", slugify(event.target.value)); }} required value={form.slug} /></div><p className={`text-xs ${effectiveSlugStatus.available === true ? "text-[var(--app-success)]" : effectiveSlugStatus.available === false ? "text-[var(--app-danger)]" : "text-[var(--app-foreground-muted)]"}`}>{effectiveSlugStatus.checking ? <LoaderCircle aria-hidden="true" className="mr-1 inline size-3 animate-spin" /> : effectiveSlugStatus.available === true ? <CheckCircle2 aria-hidden="true" className="mr-1 inline size-3" /> : null}{effectiveSlugStatus.message}</p></Field>
            <label className="flex min-h-11 items-start gap-3 text-sm leading-6"><input checked={form.termsAccepted} className="mt-1 size-4 accent-[var(--brand-700)]" onChange={(event) => update("termsAccepted", event.target.checked)} type="checkbox" /><span>Li e aceito os <Link className="-my-2 inline-flex min-h-11 items-center font-semibold text-brand-700 underline" href="/termos" target="_blank">termos de uso</Link>.</span></label>
            <label className="flex min-h-11 items-start gap-3 text-sm leading-6"><input checked={form.privacyAccepted} className="mt-1 size-4 accent-[var(--brand-700)]" onChange={(event) => update("privacyAccepted", event.target.checked)} type="checkbox" /><span>Li e aceito a <Link className="-my-2 inline-flex min-h-11 items-center font-semibold text-brand-700 underline" href="/privacidade" target="_blank">política de privacidade</Link>.</span></label>
